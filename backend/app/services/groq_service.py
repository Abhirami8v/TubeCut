"""
gemini_service.py  (now powered by Groq — llama-3.3-70b-versatile)
Finds viral clip moments from transcript using Groq's chat API.
"""
...
from app.core.config import GROQ_API_KEY, MAX_CLIP_SECONDS, MIN_CLIP_SECONDS, TARGET_CLIP_COUNT
...
def analyze_transcript(
    ...
    if GROQ_API_KEY:
        try:
            ...
            print(f"[groq_analysis] Groq analysis failed, using fallback: {exc}")
...
2. Curiosity gaps that make the viewer want to keep watching
3. Emotional stories, shocking facts, or surprising revelations
4. Clear payoff or conclusion within the clip
5. Self-contained meaning (works without watching the rest)

Rules:
- Return EXACTLY {count} clips
- Each clip must be between {MIN_CLIP_SECONDS} and {MAX_CLIP_SECONDS} seconds long
- No overlapping time ranges
- Pick the genuinely most viral moments, not just the beginning

Return ONLY valid JSON, no markdown backticks, no explanation, in this exact format:
{{
  "clips": [
    {{
      "start_time": 0.0,
      "end_time": 30.0,
      "confidence_score": 92,
      "reason": "Why this specific moment works as a standalone viral clip",
...
{transcript_json}"""
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are an expert viral video editor. You always return valid JSON only, no markdown, no explanation.",
            },
            {
                "role": "user",
                "content": prompt,
...
def _fallback_segmentation(transcript: List[dict], count: int) -> List[CandidateClip]:
    """Used only if Groq fails completely."""
    if not transcript:
        return []
